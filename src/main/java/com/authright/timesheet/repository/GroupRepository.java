...
public interface GroupRepository extends CrudRepository<Group, Long> {
    ...
    @Query("SELECT u FROM User as u left join fetch u.joinGroups as g WHERE g.groupId = :groupId order by u.userId")
    List<User> findUsers(@Param("groupId") long groupId);

    @Query("SELECT g FROM Group g order by g.groupId")
